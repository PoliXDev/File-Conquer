    ...
    def run(self):
        try:
            ...
            if self.tipo_conversion == "Excel a CSV":
                ...
                imagen = Image.open(self.ruta_entrada)
                self.progress.emit(50)
                imagen.save(self.ruta_salida, 'AVIF', quality=80)
                self.progress.emit(100)
                
            elif self.tipo_conversion == "PNG a SVG":
                # Para PNG a SVG requiere bibliotecas adicionales como potrace y pyvips
                ...
                with open(self.ruta_salida, 'w') as f:
                    f.write('<svg width="{}" height="{}" xmlns="http://www.w3.org/2000/svg">'.format(
